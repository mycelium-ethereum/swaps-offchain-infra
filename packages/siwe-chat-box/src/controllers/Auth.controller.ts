...
export const setNonceSession = async (
	req: express.Request,
...
export const verifyMsgWithNonceSession = async (
	req: express.Request,
	res: express.Response,
) => {
	try {
		...
		req.session.cookie.expires = new Date(
			fields.expirationTime || new Date(Date.now() + 30 * 60 * 1000),
		);
		...
		switch (e) {
			case ErrorTypes.EXPIRED_MESSAGE: {
				req.session.save(() =>
					res
						.status(HttpStatusCode.LOGIN_TIMEOUT)
