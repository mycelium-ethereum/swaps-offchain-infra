...
export interface IParameterService {
    createNewParameter: (key: string, value: string) => Promise<void>;
...
class ParameterService implements IParameterService {
    ...
    async createNewParameter(key: string, value: string): Promise<void> {
        try {
            ...
            const newParameter = new Parameter({ key, value });
