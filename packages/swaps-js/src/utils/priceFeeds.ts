...
import {
  knownTokenToBinanceSymbols,
  knownTokenToBitfinexSymbols,
  MAX_PRICE_FEED_TOKENS_LENGTH,
} from '../constants';

const BASE_CRYPTO_COM_URL = 'https://api.crypto.com';
export const tokensToCryptoComQuery =
  (/* tokens: LabelledToken[] */): string => {
...
export const tokensToBinanceQuery = (tokens: LabelledToken[]): string => {
  ...
  const symbolsArray = `[${filteredTokens
    .map((token) => `"${token}"`)
    .join(',')}]`;
...
export const getPriceBits = (
  prices: ethers.BigNumber[] | BigNumber[]
): TokenPriceInBits => {
