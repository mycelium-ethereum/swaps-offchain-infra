...
export const binanceSymbolToKnownToken: Record<string, KnownToken> =
  Object.keys(knownTokenToBinanceSymbols).reduce(
    (o, k) => ({
      ...o,
      [knownTokenToBinanceSymbols[k as KnownToken] as string]: k,
    }),
    {}
  );

export const createBinanceWsFeeds = (tokens: KnownToken[]) =>
  tokens.length > 0
    ? [
        {
          params: tokens.map(
            (symbol) =>
              `${knownTokenToBinanceSymbols[symbol].toLowerCase()}@ticker`
          ),
          id: 1,
        },
      ]
...
export const bitfinexSymbolToKnownToken: Partial<Record<string, KnownToken>> =
  Object.keys(knownTokenToBitfinexSymbols).reduce(
    (o, k) => ({
      ...o,
...
export const createBitfinexWsFeeds = (tokens: KnownToken[]) => {
  const bitfinexTokens = tokens.filter(
    (token) => !!knownTokenToBitfinexSymbols[token]
  );
...
export const ftxSymbolToKnownToken: Record<string, KnownToken> = Object.keys(
  knownTokenToFTXSymbols
).reduce(
  (o, k) => ({ ...o, [knownTokenToFTXSymbols[k as KnownToken]]: k }),
  {}
);

export const createFtxWsFeeds = (tokens: KnownToken[]) =>
  tokens.map((market) => ({
    channel: 'ticker' as const,
...
export const cryptoComSymbolToKnownToken: Partial<Record<string, KnownToken>> =
  Object.keys(knownTokenToCryptoComSymbols).reduce(
    (o, k) => ({
      ...o,
...
export const createCryptoComWsFeeds = (tokens: KnownToken[]) => {
  const cryptoComTokens = tokens.filter(
    (token) => !!knownTokenToCryptoComSymbols[token]
  );
  if (tokens.length === 0) return [];
  return [
    {
      params: {
        channels: cryptoComTokens.map(
          (symbol) => `ticker.${knownTokenToCryptoComSymbols[symbol]}`
        ),
      },
    },
  ];
...
export const coinbaseSymbolToKnownToken: Partial<Record<string, KnownToken>> =
  Object.keys(knownTokenToCoinbaseSymbols).reduce(
    (o, k) => ({
      ...o,
...
export const createCoinbaseWsFeeds = (
  tokens: KnownToken[]
): CoinbaseWsTopic[] => {
  const coinbaseTokens = tokens.filter(
    (token) => !!knownTokenToCoinbaseSymbols[token]
  );
  if (tokens.length === 0) return [];
  return [
    {
      channels: ['ticker'],
      product_ids: coinbaseTokens.map(
        (symbol) => knownTokenToCoinbaseSymbols[symbol] as string
      ),
    },
  ];
};
