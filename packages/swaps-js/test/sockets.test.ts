  ...
  describe('Update message handling', () => {
    test('Parse update message', () => {
      const testUpdateMessage = (
        wsKey: WsKey,
        socket: any,
        expectedToken?: KnownToken
      ) => {
        const updateMessage = constructUpdateMessage(
          wsKey,
          '999',
          '1000',
          '1000'
        ...
        expect(result.price.toString()).toEqual(
          ethers.utils.parseEther('1000').toString()
        );
        expect(result.lastPrice.toString()).toEqual(
      ...
      bitfinexSocket.wsStore.verifySubscription(
        'bitfinex',
        '1',
        KnownToken.ETH
      );
    ...
    test('Incomplete update message', () => {
      const testUpdateMessage = (wsKey: WsKey, socket: any) => {
        ...
        let updateMessage = constructUpdateMessage(
          wsKey,
          hiddenNull,
          '1000',
          '1000'
        ...
        updateMessage = constructUpdateMessage(
          wsKey,
          '1000',
          hiddenNull,
          '1000'
    ...
    test('Unknown in subscription message token', () => {
      const testSubscriptionMessage = (
        ...
        if (expectSubscription) {
          ...
          expect(error.type).toEqual('ON_WS_MESSAGE');
          expect(error.error.toString()).toEqual(
            'Error: Unknown token: unknownTokenSymbol'
          );
          expect(verifiedSubscriptions).toEqual({});
        } else {
          expect(result).toBeTruthy();
