...
const expectedTokenFeeds = {
  ...
  all: {
    ...
    bitfinex: bitfinexFeeds.map((symbol) => ({ channel: 'ticker', symbol })),
...
describe('Creates appropriate feeds', () => {
  ...
  test('Some tokens', () => {
    ...
    expect(cryptoComFeeds).toEqual([
      expectedTokenFeeds[KnownToken.FXS].cryptoCom,
    ]);
  });
  test('All possible tokens', () => {
    const tokens: KnownToken[] = Object.values(
      KnownTokenMap[NETWORK.ARBITRUM_MAINNET]
    );
    const binanceFeeds = createBinanceWsFeeds(tokens);
    const ftxFeeds = createFtxWsFeeds(tokens);
    const bitfinexFeeds = createBitfinexWsFeeds(tokens);
    ...
    expect(binanceFeeds[0].params.sort()).toEqual(
      expected.binance[0].params.sort()
    );
    ...
    expect(cryptoComFeeds[0].params.channels.sort()).toEqual(
      expected.cryptoCom[0].params.channels.sort()
    );
  });
});
