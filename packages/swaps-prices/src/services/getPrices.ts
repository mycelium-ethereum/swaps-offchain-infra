...
const cachedPrices: Record<
  string,
  { expiry: number; tokens: Record<string, string> }
> = {};
...
export const getPrices = async ({ network }: GetPriceArgs) => {
  ...
  knownTokens.forEach((token) => {
    ...
    if (priceStore.medianPrices[token.knownToken]) {
      ...
      const priceAges = Object.values(allPrices).map((price) =>
        ethers.BigNumber.from(price.updated)
      );
      ...
      tokens[token.address] = {
        price: ethers.utils
          .parseUnits(medianPrice.toString(), 12 /* 30 - 18 */)
          .toString(),
