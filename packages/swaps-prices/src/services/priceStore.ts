...
class PriceStore {
  prices: Partial<
    Record<
      KnownToken,
      Record<string, { price: ethers.BigNumber; updated: number }>
    >
  > = {};
  ...
  public updateMedianPrice(token: KnownToken) {
    ...
    const prices: ethers.BigNumber[] = Object.values(cexPrices).map(
      (prices) => prices.price
    );
    ...
    const medianPriceChanged =
      previousMedianPrice && !previousMedianPrice.eq(medianPrice);
