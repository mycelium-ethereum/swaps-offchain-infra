...
const startPingingConnectedClients = (intervalMs: number = 60 * 1000) =>
  setInterval(checkClients, intervalMs);
...
cryptoComClient.on('update', (data) =>
  priceStore.storePrice('cryptoCom', data)
);
...
const subscribeWsFeeds = () => {
  const tokens: KnownToken[] = networkTokens[NETWORK.ARBITRUM_MAINNET].map(
