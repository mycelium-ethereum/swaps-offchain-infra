 ...
 */
export const handleClosedConnection = (wsProvider_: ethers.providers.WebSocketProvider, handleClosedConnection: (newProvider: ethers.providers.WebSocketProvider) => any) => {
  ...
  const init = () => {
    const startPinging = () => {
        keepAliveInterval = setInterval(() => {
          logger.info(`Checking if the connection is alive with ping, will close the connection after ${EXPECTED_PONG_BACK}ms`, { url })
    ...
    wsProvider._websocket.on('pong', () => {
      logger.info(`Received pong within ${EXPECTED_PONG_BACK}ms, so connection is alive, clearing the timeout`, { url })
      clearInterval(pingTimeout)
    })

    wsProvider._websocket.on('close', async (code: any) => {
        logger.error('The websocket provider connection was closed', { code, url })
