...
export const fallbackProvider = fallbackRPC
  ? createProvider(fallbackRPC)
  : undefined;
...
export const callContract = async (
  ...
  try {
    ...
    if (fallbackProvider) {
      logger.info(
        `Trying ${label} with fallback provider, promise will timeout after 3 seconds`
      );
      try {
...
export const handleClosedConnection = (
  ...
  handleClosedConnection: (
    newProvider: ethers.providers.WebSocketProvider
  ) => any
) => {
  ...
  const init = () => {
    ...
    wsProvider._websocket.on('pong', () => {
      logger.info(
        `Received pong within ${EXPECTED_PONG_BACK}ms, so connection is alive, clearing the timeout`,
        { url }
      );
