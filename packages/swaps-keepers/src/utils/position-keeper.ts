...
export async function getRequestQueueLengths (positionRouter: PositionRouter): Promise<QueueLength> {
    ...
    const queueLengths: ethers.BigNumber[] = await callContract(positionRouter, 'getRequestQueueLengths', [], 'positionRouter.getRequestQueueLengths()')
    return ({
...
export function getGreedyQueueLengths ( maxExecutableChunk: number, lastQueueLength?: QueueLength): QueueLength | undefined {
    if (lastQueueLength) {
        return ({
            ...lastQueueLength,
