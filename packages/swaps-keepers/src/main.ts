...
if (!process.env.POSITION_ROUTER) {
  console.log(
    'Must provide POSITION_ROUTER address as an environment variable'
  );
  process.exit(0);
}
...
const maxExecutionRetry = process.env.MAX_EXECUTION_RETRY
  ? Number(process.env.MAX_EXECUTION_RETRY)
  : 5;
const maxExecutableChunk = process.env.MAX_EXECUTABLE_CHUNK
  ? Number(process.env.MAX_EXECUTABLE_CHUNK)
  : 100;
...
const priceKeeperInterval = process.env.PRICE_KEEPER_INTERVAL_MS
  ...
  : 20000; // default 20 seconds
const forceUpdateInterval = process.env
  .PRICE_KEEPER_FORCE_UPDATE_INTERVAL_SECONDS
...
const initNewPositionRouter = (
  ...
  if (!pausedPositionKeeper) {
    ...
    orderEmitter.on('increasePosition', (positionInfo) =>
      positionKeeper.handleIncreasePosition(
        priceFeed,
        fastPriceContract,
        positionRouter,
        positionInfo
    ...
    orderEmitter.on('decreasePosition', (positionInfo) =>
      positionKeeper.handleDecreasePosition(
        priceFeed,
        fastPriceContract,
        positionRouter,
        positionInfo
...
const main = async () => {
  ...
  const signer = new ethers.Wallet(
    process.env.SIGNING_PRIVATE_KEY as string,
    provider
  );
  ...
  const feedContract = VaultPriceFeed__factory.connect(
    priceFeedAddress,
    provider
  );
  const secondaryPriceFeed = await feedContract.secondaryPriceFeed();
  let fastPriceContract = FastPriceFeed__factory.connect(
    secondaryPriceFeed,
    signer
  );
  ...
  let positionRouter = initNewPositionRouter(
    priceFeed,
    fastPriceContract,
    positionKeeper,
    provider
  ...
  if (isWsProvider(rpc)) {
    handleClosedConnection(
      provider as ethers.providers.WebSocketProvider,
      async (newProvider) => {
        logger.info(
          `Reconnecting websocket provider: ${newProvider.connection.url}`
        );
        const providerIsHealthy = await checkProviderHealth(newProvider);
        if (providerIsHealthy) {
          // connect new provider
          const newFastPriceContract = fastPriceContract.connect(
  ...
  if (!pausedPriceKeeper) {
    ...
    const [binancePrices, bitFinexPrices, cryptoComPrices /* , ftxPrices */] =
      await Promise.all([
        pricePoller
          .fetchBinancePrices()
          .catch((error) => pricePollerError(error, 'binance')),
        pricePoller
          .fetchBitfinexPrices()
          .catch((error) => pricePollerError(error, 'bitfinex')),
        // pricePoller.fetchFTXPrices().catch((error) => pricePollerError(error, 'ftx')),
        pricePoller
          .fetchCryptoComPrices()
          .catch((error) => pricePollerError(error, 'cryptoCom')),
      ]);
    ...
    asyncInterval({
      fn: async () => {
        logger.info(
          `Checking stale prices, will update if prices are older than ${forceUpdateInterval}s`
        );
        priceKeeper.checkStalePrices(
          priceFeed,
          fastPriceContract,
          forceUpdateInterval
        );
        ...
        logger.info(
          'Updating CEX prices, fetching binance, bitfinex, crypto.com and feedPrices'
        );
        const [
          binancePrices,
          bitFinexPrices,
          cryptoComPrices /*, ftxPrices */,
          feedPrices,
        ] = await Promise.all([
          pricePoller
            .fetchBinancePrices()
        ...
        logger.info(
          `Finished checking and updating prices, sleeping for ${priceKeeperInterval}s`
        );
      },
      ...
      onUnavailable: () => {
        logIntervalError(
          'Tried checking stale prices but interval mutex unavailable'
        );
      },
    });
    ...
    priceKeeper.on('force_update', ({ lastUpdatedAt, now }) => {
      logger.info(
        `Prices are older than ${forceUpdateInterval}, forcing update`,
        { lastUpdatedAt, now, forceUpdateInterval }
      );
  ...
  if (!pausedPositionKeeper) {
    ...
    asyncInterval({
      fn: async () => {
        ...
        await positionKeeper.executeOutstandingOrders(
          priceFeed,
          fastPriceContract,
          positionRouter
        );
        logger.info(
          `Finished checking outstanding orders, sleeping for ${positionKeeperInterval}ms`
        );
      },
      ...
      onUnavailable: () => {
        logIntervalError(
          'Tried checking outstanding orders but interval mutex unavailable'
        );
      },
    });
