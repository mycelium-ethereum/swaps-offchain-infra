...
const positionKeeperInterval = process.env.POSITION_KEEPER_INTERVAL_MS ? parseInt(process.env.POSITION_KEEPER_INTERVAL_MS) : 20000; // default 20 seconds
const pausedPositionKeeper = process.env.POSITION_KEEPER_PAUSED === 'true';

// priceKeeper props
const priceKeeperInterval = process.env.PRICE_KEEPER_INTERVAL_MS ? parseInt(process.env.PRICE_KEEPER_INTERVAL_MS) : 20000; // default 20 seconds
const forceUpdateInterval = process.env.PRICE_KEEPER_FORCE_UPDATE_INTERVAL_SECONDS ? parseInt(process.env.PRICE_KEEPER_FORCE_UPDATE_INTERVAL_SECONDS) : 60 * 5; // default 5 minutes
...
const logResult = (message: string, result: UpdateResult) => {
    const prices = result.prices.map(({ knownToken, price }) => ({ knownToken, price: price?.toString() }))
...
const initNewPositionRouter = (priceFeed: PriceFeed, fastPriceContract: FastPriceFeed, positionKeeper: PositionKeeper, provider: ethers.providers.Provider, oldProvider?: ethers.providers.Provider): PositionRouter => {
  if (oldProvider) {
  ...
  if (!pausedPositionKeeper) {
    ...
    orderEmitter.on('increasePosition', (positionInfo) => positionKeeper.handleIncreasePosition(priceFeed, fastPriceContract, positionRouter, positionInfo))
    orderEmitter.on('decreasePosition', (positionInfo) => positionKeeper.handleDecreasePosition(priceFeed, fastPriceContract, positionRouter, positionInfo))
  } else {
...
const main = async () => {
    ...
    const priceFeed = await PriceFeed.Create({ priceFeed: priceFeedAddress, signer, shadowMode })
    ...
    if (isWsProvider(rpc)) {
      handleClosedConnection(provider as ethers.providers.WebSocketProvider, async (newProvider) => {
        ...
        if (providerIsHealthy) {
          ...
          positionRouter = initNewPositionRouter(priceFeed, newFastPriceContract, positionKeeper, newProvider, provider)
        } else {
    ...
    if (!pausedPriceKeeper) {
      ...
      priceKeeper.on('force_update', ({ lastUpdatedAt, now }) => {
        logger.info(`Prices are older than ${forceUpdateInterval}, forcing update`, { lastUpdatedAt, now, forceUpdateInterval });
    ...
    if(!pausedPositionKeeper) {
      ...
      asyncInterval({
        fn: async () => {
          logger.info("Checking outstanding orders, execution will trigger if startIndex(Increase/Decrease)Position != endIndex(Increase/Decrease)Position")
