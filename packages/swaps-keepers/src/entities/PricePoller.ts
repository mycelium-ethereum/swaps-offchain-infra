...
type KnownSymbols = typeof bitfinexSymbolToKnownToken | typeof ftxSymbolToKnownToken | typeof binanceSymbolToKnownToken | typeof cryptoComSymbolToKnownToken;
...
export class PricePoller {
  ...
  fetchBinancePrices = async (): Promise<ParsedTokenPrice[]> => {
    ...
    const parsedBinancePrices = this.parseAndOrderResults<BinancePrice>(binancePrices, binanceSymbolToKnownToken, (r) => ({ symbol: r.symbol, bestBid: r.price, bestAsk: r.price}));
  ...
  fetchCryptoComPrices = async (): Promise<ParsedTokenPrice[]> => {
    ...
    const parsedCryptoComPrices = this.parseAndOrderResults<CryptoComPrice>(filteredCryptoComPrices, cryptoComSymbolToKnownToken, (r) => ({ symbol: r.i, bestBid: r.b, bestAsk: r.a }));
  ...
  fetchFTXPrices = async (): Promise<ParsedTokenPrice[]> => {
    ...
    const parsedFTXPrices = this.parseAndOrderResults<FTXPrice>(filteredFTXPrices, ftxSymbolToKnownToken, (r) => ({ symbol: r.name, bestBid: r.bid, bestAsk: r.ask }));
  ...
  fetchBitfinexPrices = async (): Promise<ParsedTokenPrice[]> => {
    const bitfinexPrices = await fetch(tokensToBitfinexQuery(this.tokens))
      ...
      const parsedBitfinexPrices = this.parseAndOrderResults<BitfinexPrice>(bitfinexPrices, bitfinexSymbolToKnownToken, (r) => ({ symbol: r[bitfinexSymbolIndex], bestBid: r[bitfinexBestBidPriceIndex], bestAsk: r[bitfinexBestAskPriceIndex]}));
   ...
   */
  parseAndOrderResults= <P>(prices: P[], knownSymbols: KnownSymbols, destructure: (r:P) => { symbol: string, bestAsk: string | number, bestBid: string | number }): ParsedTokenPrice[] => {
