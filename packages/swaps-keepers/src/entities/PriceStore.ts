...
interface PriceEvents {
  'exceeded': ({ knownToken, feedPrice, medianPrice }: { knownToken: KnownToken, medianPrice: string, feedPrice: string }) => void;
}

const DEFAULT_PRICE_THRESHOLD = process.env.PRICE_THRESHOLD ? ethers.utils.parseEther(process.env.PRICE_THRESHOLD) : ethers.utils.parseEther('0.0012');
...
export class PriceStore extends TypedEmitter<PriceEvents> {
  ...
  storePrice(key: string, tokenPrice: ParsedTokenPrice, noCompare?: boolean) {
    ...
    if (!price) {
      logger.warn(`Tried storing ${key} price`, { knownToken, price, key })
      return
    }
  ...
  storePrices(key: string, prices: ParsedTokenPrice[], noCompare?: boolean) {
    logger.info(`Storing ${key} prices`, { key, prices: prices.map(({ knownToken, price }) => ({ knownToken, price: price ? ethers.utils.formatEther(price) : undefined }))})
  ...
  priceCompare(knownToken: KnownToken): void {
    ...
    const diff = 
      ((feedPrice.sub(medianPrice))
      .mul(ethers.utils.parseEther('1')))
      .div(medianPrice)

    const threshold = assetThresholds[knownToken] ?? DEFAULT_PRICE_THRESHOLD;
    if (diff.abs().gt(threshold)) {
      this.emit('exceeded', { knownToken, feedPrice: feedPrice.toString(), medianPrice: medianPrice.toString() })
    }
  }
