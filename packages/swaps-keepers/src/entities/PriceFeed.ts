...
export default class PriceFeed  {
  ...
  public async init (props: IPriceFeed): Promise<void> {
    ...
    logger.info('Initiated PriceFeed', { tokens: priceFeedTokens, network })
    // order is preserved
   ...
   */
  public async updatePricesWithBits (fastPriceContract: FastPriceFeed, prices: ParsedTokenPrice[]): Promise<UpdateResult | undefined> {
   ...
   * @returns the txn hash of the update
   */
  public async updatePricesWithBitsAndExecute (fastPriceContract: FastPriceFeed, prices: ParsedTokenPrice[], increaseIndex: number, decreaseIndex: number): Promise<UpdateResult | undefined> {
    return this.updatePrices(fastPriceContract, 'setPricesWithBitsAndExecute', prices, [increaseIndex, decreaseIndex])
  }

  private async updatePrices (fastPriceContract: FastPriceFeed, fn: 'setPricesWithBits' | 'setPricesWithBitsAndExecute', prices: ParsedTokenPrice[], extraArgs: any[] = []): Promise<UpdateResult | undefined> {
    // format prices with their respective precision
    const { priceInBits, timestamp } = this.prepareUpdateArgs(prices);

    logger.info(`Attempting to ${fn}`, { priceInBits, prices: prices.map(({ knownToken, price }) => ({ knownToken, price: price.toString() })), timestamp, ...extraArgs })

    try {
      if (!this.shadowMode) {
        let txnReceipt = await this._attemptToExecute(fastPriceContract, fn, priceInBits, timestamp, extraArgs, false);
        if (!txnReceipt) {
          ...
          if (fallbackFastFeed) {
              ...
              txnReceipt = await this._attemptToExecute(fallbackFastFeed, fn, priceInBits, timestamp, extraArgs, true)
          }
        }
        if (txnReceipt) {
          const lastUpdatedAt = await this.updateLastUpdatedAt(fastPriceContract);
          return ({ txnHash: txnReceipt.transactionHash, prices, timestamp, lastUpdatedAt });
        }
      } else {
   ...
   * Attempts to execute
   */
  private async _attemptToExecute (fastPriceContract: FastPriceFeed, fn: 'setPricesWithBits' | 'setPricesWithBitsAndExecute', priceInBits: string, timestamp: number, extraArgs: any[] = [], usingFallback: boolean): Promise<ethers.providers.TransactionReceipt | undefined> {
    ...
    const txnReceipt = await attemptPromiseRecursively<ethers.providers.TransactionReceipt | undefined>({
      promise: async () => {
        // @ts-ignore
        const txn = await fastPriceContract[fn](priceInBits, timestamp, ...extraArgs);
   ...
   */
  public prepareUpdateArgs (prices: ParsedTokenPrice[]): { priceInBits: string, timestamp: number } {
  ...
  public async fetchFeedPrices (fastPriceContract: FastPriceFeed): Promise<ParsedTokenPrice[]> {
    const primaryPrices: ethers.BigNumber[] = await Promise.all(this.tokens.map(async (token) => callContract(fastPriceContract, 'prices', [token.address], `fastPriceContract.prices(${token.address}:${token.knownToken})`))) as ethers.BigNumber[]
