...
export default class PriceKeeper extends TypedEmitter<PriceKeeperEvents> {
  ...
  async updatePrices(
    ...
    const result = await priceFeed.updatePricesWithBits(
      fastFeedContract,
      medianPrices
    );
    if (result) {
  ...
  async checkStalePrices(
    priceFeed: PriceFeed,
    fastFeedContract: FastPriceFeed,
    forceUpdateInterval: number
  ) {
    const lastUpdatedAt =
      (await priceFeed.updateLastUpdatedAt(fastFeedContract)) ?? 0;
    const now = Math.floor(Date.now() / 1000);
    const priceAge = now - lastUpdatedAt;
    logger.info(
      `Prices are ${priceAge}s old, last updated at ${lastUpdatedAt}`
    );
