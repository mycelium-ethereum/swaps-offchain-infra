...
export class PricePoller {
  ...
  fetchBinancePrices = async (): Promise<ParsedTokenPrice[]> => {
    const binancePrices: BinancePrice[] = (await fetch(
      tokensToBinanceQuery(this.tokens)
    )
      .then((res) => res.json())
      .catch((err) => {
  ...
  fetchCryptoComPrices = async (): Promise<ParsedTokenPrice[]> => {
    const cryptoComPrices = (await fetch(
      tokensToCryptoComQuery(/* this.tokens */)
    )
      .then((res) => res.json())
      .catch((err) => {
    ...
    const filteredCryptoComPrices = cryptoComPrices.result.data.filter(
      (cryptoComResult: CryptoComPrice) => {
  ...
  fetchFTXPrices = async (): Promise<ParsedTokenPrice[]> => {
    ...
    const parsedFTXPrices = this.parseAndOrderResults<FTXPrice>(
      filteredFTXPrices,
      ftxSymbolToKnownToken,
      (r) => ({ symbol: r.name, bestBid: r.bid, bestAsk: r.ask })
    );
  ...
  fetchMedianPrices = async (): Promise<TokenPrices> => {
    const [binancePrices, ftxPrices, bitFinexPrices, cryptoComPrices] =
      await Promise.all([
    ...
    if (
      ...
      logger.info(
        'Failed to fetch median token prices: Not all tokens are set'
      );
      return [];
    }
  ...
  fetchMedianPricesAsObject = async (): Promise<
    Record<string, ethers.BigNumber>
  > => {
    const medianPrices = await this.fetchMedianPrices();
    return medianPrices.reduce(
      (o, price, i) => ({ ...o, [this.tokens[i].address]: price }),
      {}
    );
  };
  ...
  parseAndOrderResults = <P>(
    ...
    if (parsedResults.length !== this.tokens.length) {
      this.tokens.forEach((token) => {
        const tokenPrice = parsedResults.find(
          (token_) => token_.knownToken === token.knownToken
        );
        if (!tokenPrice) {
          parsedResults.push({
            // @ts-ignore
