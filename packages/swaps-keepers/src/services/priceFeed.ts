...
export default class PriceFeed {
  ...
  public async init(props: IPriceFeed): Promise<void> {
    ...
    const fastPriceContract = FastPriceFeed__factory.connect(
      secondaryPriceFeed,
      signer
    );
    ...
    const tokens = await Promise.all(
      indexes.map((i) => fastPriceContract.tokens(i))
    );
    const tokenPrecisions = await Promise.all(
      indexes.map((i) => fastPriceContract.tokenPrecisions(i))
    );
    ...
    try {
      if (!this.shadowMode) {
        ...
        if (!txnReceipt) {
          ...
          const fallbackFastFeed =
            this.connectSecondaryProvider(fastPriceContract);
          if (fallbackFastFeed) {
        ...
        if (txnReceipt) {
          const lastUpdatedAt = await this.updateLastUpdatedAt(
            fastPriceContract
          );
          return {
        ...
        logger.info(
          `Keeper in shadow mode: sleeping for 10 seconds to simulate order execution`
        );
        await delay(10 * 1000);
      }
    } catch (error) {
  ...
  private async _attemptToExecute(
    ...
    const usingProvider = `using ${
      usingFallback ? 'fallback' : 'primary'
    } provider`;

    const txnReceipt = await attemptPromiseRecursively<
    ...
    >({
      ...
      logError(
        `Failed executing ${fn} ${usingProvider}. Will not retry`,
        error
      );
      return undefined;
    });
   ...
   */
  public async fetchFeedPrices(
    fastPriceContract: FastPriceFeed
  ): Promise<ParsedTokenPrice[]> {
  ...
  public async updateLastUpdatedAt(
    fastPriceContract: FastPriceFeed
  ): Promise<number> {
    const lastUpdatedAt = await callContract(
      fastPriceContract,
      'lastUpdatedAt',
      [],
      'fastPriceFeed.lastUpdatedAt'
    );
    this.lastUpdatedAt = (lastUpdatedAt as ethers.BigNumber).toNumber();
    return this.lastUpdatedAt;
  }

  public connectSecondaryProvider(
    contract: FastPriceFeed
  ): FastPriceFeed | undefined {
    if (this.signer && fallbackProvider) {
      return contract.connect(this.signer.connect(fallbackProvider));
    } else {
      logger.warn(
        'Tried connecting secondary provider but no fallback provider set'
      );
      return;
    }
  }
