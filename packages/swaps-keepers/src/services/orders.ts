...
import { PositionInfo } from "../types/position-keeper";
...
function handleIncreasePosition (
    ...
    orderEmitter.emit('increasePosition', { account, path, indexToken, isLong, blockNumber: txnInfo.blockNumber })
}
...
function handleDecreasePosition(
    ...
    orderEmitter.emit('decreasePosition', { account, path, indexToken, isLong, blockNumber: txnInfo.blockNumber })
}
...
function streamOrders(positionRouter: PositionRouter): void {
    console.info("Starting to stream CreateIncrease/DecreasePosition and ExecuteIncrease/DecreasePosition positionRouter events")
