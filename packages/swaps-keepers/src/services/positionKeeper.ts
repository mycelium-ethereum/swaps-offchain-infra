...
import {
  ...
  getExecutableEndIndex,
  getGreedyQueueLengths,
} from '../utils';
  ...
  async executeOutstandingOrders(
    ...
    if (
      ...
      if (this.shouldExecute()) {
        ...
        logger.warn(
          'Skipped outstanding order execution: already processing order execution'
        );
      }
    }
    ...
    if (medianPrices && priceFeed) {
      ...
      if (
        queueLengths.startIndexForIncreasePositions !==
          queueLengths.endIndexForIncreasePositions ||
        queueLengths.startIndexForDecreasePositions !==
          queueLengths.endIndexForDecreasePositions
      ) {
