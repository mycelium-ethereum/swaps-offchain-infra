...
export default class DecreaseOrderService {
  public async createFromEvent(event: LogDescription) {
    ...
    if (!foundOrder) {
      ...
      await order.save();
      console.log(
        `Created DecreaseOrder ${event.args.account}:${event.args.orderIndex}`
      );
      return order;
    } else {
      console.log(
        `DecreaseOrder ${event.args.account}:${event.args.orderIndex} already exists`
      );
      return foundOrder;
    }
  }
  ...
  public async updateFromEvent(event: LogDescription) {
    ...
    if (order) {
      ...
      await order.save();
      console.log(
        `Updated DecreaseOrder ${event.args.account}:${event.args.orderIndex}`
      );
    }
  }
