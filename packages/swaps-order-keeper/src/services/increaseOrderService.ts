...
export default class IncreaseOrderService {
  public async createFromEvent(event: LogDescription) {
    ...
    if (!foundOrder) {
      ...
      await order.save();
      console.log(
        `Created IncreaseOrder ${event.args.account}:${event.args.orderIndex}`
      );
      return order;
    } else {
      console.log(
        `IncreaseOrder ${event.args.account}:${event.args.orderIndex} already exists`
      );
      return foundOrder;
    }
  }
  ...
  public async updateFromEvent(event: LogDescription) {
    ...
    if (order) {
      ...
      console.log(
        `Updated IncreaseOrder ${event.args.account}:${event.args.orderIndex}`
      );
      return order;
    }
  }
