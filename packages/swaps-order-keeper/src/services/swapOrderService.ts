...
export default class SwapOrderService {
  public async createFromEvent(event: LogDescription) {
    ...
    if (!foundOrder) {
      ...
      await order.save();
      console.log(
        `Created SwapOrder ${event.args.account}:${event.args.orderIndex}`
      );
      return order;
    } else {
      console.log(
        `SwapOrder ${event.args.account}:${event.args.orderIndex} already exists`
      );
      return foundOrder;
    }
  }
  ...
  public async updateFromEvent(event: LogDescription) {
    ...
    if (order) {
      ...
      await order.save();
      console.log(
        `Updated SwapOrder ${event.args.account}:${event.args.orderIndex}`
      );
      return order;
    }
  }
