...
export const getUsdgMinPrice = async (
  vault: Vault,
  otherToken: string
): Promise<BigNumber> => {
  ...
  const redemptionAmount = await vault.getRedemptionAmount(
    otherToken,
  ...
  const price = redemptionAmount
    .mul(otherTokenPrice)
    .div(ethers.utils.parseUnits("1", otherTokenDecimals));
  cache.set(cacheKey, price.toString());
  return price;
};
