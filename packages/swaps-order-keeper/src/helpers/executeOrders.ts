...
async function executeOrderBatch(orders: OrderList, signer: ethers.Signer) {
    ...
    if (validOrdersToExecute !== 0) {
        ...
        const numExecuted = validOrdersToExecute - errorEvents.length;
        ...
        console.log(colors.yellow(`No valid orders to execute`));
    }
}
...
function shouldBeDeleted(ev: ethers.Event): boolean {
    if (ev.event === "ExecuteDecreaseOrderError" && ev.args.reason === "Vault: empty position") {
        return true;
    }
