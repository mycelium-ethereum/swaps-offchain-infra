...
async function executeOrderBatch(orders: OrderList, signer: ethers.Signer) {
  if (orders.total() > ORDERS_PER_TRANSACTION) {
    throw new Error(
      `Cannot execute more than ${ORDERS_PER_TRANSACTION} orders per transaction`
    );
  }

  const positionManager = PositionManager__factory.connect(
    process.env.POSITION_MANAGER_ADDRESS,
    signer
  );
  const feeReceiver = process.env.FEE_RECEIVER_ADDRESS;

  let incAccounts: string[] = [];
  let incOrderIndices: number[] = [];
  await Promise.all(
    orders.increaseOrders.map(async (order) => {
      try {
        await positionManager.estimateGas.executeIncreaseOrder(
          order.account,
          order.orderIndex,
          feeReceiver
        );
  ...
  let decAccounts: string[] = [];
  let decOrderIndices: number[] = [];
  await Promise.all(
    orders.decreaseOrders.map(async (order) => {
      try {
        await positionManager.estimateGas.executeDecreaseOrder(
          order.account,
          order.orderIndex,
          feeReceiver
        );
  ...
  let swapAccounts: string[] = [];
  let swapOrderIndices: number[] = [];
  await Promise.all(
    orders.swapOrders.map(async (order) => {
      try {
        await positionManager.estimateGas.executeSwapOrder(
          order.account,
          order.orderIndex,
          feeReceiver
        );
  ...
  const errorEvents =
    receipt.events?.filter((ev) => {
      return (
        [
          "ExecuteIncreaseOrderError",
          "ExecuteDecreaseOrderError",
          "ExecuteSwapOrderError",
        ].includes(ev.event) && ev.args.account !== ethers.constants.AddressZero
      );
    }) || [];
...
function shouldBeDeleted(ev: ethers.Event): boolean {
  if (
    ev.event === "ExecuteDecreaseOrderError" &&
    ev.args.reason === "Vault: empty position"
  ) {
    return true;
  }
