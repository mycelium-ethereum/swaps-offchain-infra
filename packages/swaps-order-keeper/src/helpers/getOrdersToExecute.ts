...
export const getOrdersToExecute = async (
  ...
  for (
    let order = await incCursor.next();
    order != null;
    order = await incCursor.next()
  ) {
  ...
  for (
    let order = await decCursor.next();
    order != null;
    order = await decCursor.next()
  ) {
    const currentPrice = await getPrice(vault, order.indexToken, !order.isLong);
    const triggerPrice = BigNumber.from(order.triggerPrice);
  ...
  for (
    let order = await swapCursor.next();
    order != null;
    order = await swapCursor.next()
  ) {
