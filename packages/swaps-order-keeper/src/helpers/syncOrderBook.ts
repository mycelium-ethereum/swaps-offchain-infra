...
export const syncOrderBook = async (provider: ethers.providers.Provider) => {
  ...
  let startBlock = process.env.START_BLOCK
    ...
    : 0;
  const lastSyncedBlock = await parameterService.getParameter(
    "lastSyncedBlock"
  );
  if (lastSyncedBlock) {
    ...
    for (const log of logs) {
      ...
      switch (event.name) {
        ...
        case "CancelIncreaseOrder":
          await increaseOrderService.delete(
            event.args.account,
            event.args.orderIndex
          );
          break;
        ...
        case "CancelDecreaseOrder":
          await decreaseOrderService.delete(
            event.args.account,
            event.args.orderIndex
          );
          break;
